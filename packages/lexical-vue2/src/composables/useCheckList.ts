import {
  $getNearestNodeFromDOMNode, $getSelection, $isElementNode, $isRangeSelection,
  COMMAND_PRIORITY_LOW, KEY_ARROW_DOWN_COMMAND, KEY_ARROW_LEFT_COMMAND,
...
import { $isListItemNode, $isListNode, INSERT_CHECK_LIST_COMMAND, insertList, ListItemNode } from '@lexical/list'
...
function useCheckList() {
            ...
            if ($isRangeSelection(selection) && selection.isCollapsed()) {
              ...
              if (isElement || anchor.offset === 0) {
                ...
                if (
                  ...
                  if (domNode != null && domNode.getAttribute('role') === 'checkbox' && document.activeElement !== domNode) {
                    domNode.focus()
                    event.preventDefault()
                    return true
                  }
  ...
  function listenPointerDown() {
    if (listenersCount++ === 0) {
      editor.getRootElement()?.addEventListener('click', handleClick)
      editor.getRootElement()?.addEventListener('pointerdown', handlePointerDown)
    }
  ...
  function findEditor(target: Node): LexicalEditor | null {
